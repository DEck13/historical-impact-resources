...
In this technical report we go through the steps that we took to rank historical figures via an era-neutral version of the Historical Popularity Index (HPI) in [Yu et al (2016)](https://www.nature.com/articles/sdata201575) obtained via Full House Modeling. This technical report is reproducible by anyone who has the fullhouse package and our data sets. Long code chunks that create plots will be suppressed. Code for these plots is available in the accompanying .Rmd file. This report will cover the following topics:

# List of Topics {#topics}

- [Data Preparation](#Data): Data preprocessing steps and software
- [Historical Eras](#Eras): Definition of eras used to construct the eligible population of historic figures
  - [Eras Defined by Centuries](#Centuries): ie, all eligible people born during 1700-1799
  - [Eras Defined as Separate Chapters in a History Course](#History): ie, all eligible people born during the Emergence of the First Global Age, 1450-1770 CE
- [Full House Modeling](#FHM): Definition, implementation, diagnostics, and results of Full House Modeling
  - [Eras Defined by Centuries](#FHM-Centuries): Full House Modeling when eras are defined by centuries
  - [Eras Defined as Separate Chapters in a History Course](#FHM-History): Full House Modeling when eras are defined by chapters in a history course
- [Comparison of Approaches](#Comparison): Comparison of rankings obtained from HPI and versions of era-adjusted HPI
- [Interesting Changes in Rankings](#Changes): Additional interesting rankings changes between HPI and versions of era-adjusted HPI
  - [Eras Defined as Centuries](#Changes-Centuries): Interesting changes in rankings when switching from HPI to era-adjusted HPI with eras defined by centuries
  - [Eras Defined as Separate Chapters in a History Course](#Changes-History): Interesting changes in rankings when switching from HPI to era-adjusted HPI with eras defined by chapters in a history course
- [Sensitivity in Eligible Population](#Sensitivity-Eligible): Investigations into the sensitivity of the eligible population when eras are defined by centuries
  - [Linear Increase in Eligible Population](#Linear-Increase): Eligible population increases linearly
  - [Flat Increase in Eligible Population](#Flat-Increase): Eligible population increases linearly but is mostly flat
...
We now define historical eras. Here the eras constitute a subpopulation of contemporaneous people. People within a common era are viewed as "competing against each other" for large HPI values. [Back to topics](#topics)


## Eras Defined by Centuries {#Centuries}
...
## Eras Defined as Separate Chapters in a History Course {#History}
...
## Eras Defined by Centuries {#FHM-Centuries}
...
```{r}
kable(hpi_adjust_vs_peers %>% as.data.frame() %>% head(50), 
      format = "html") %>% 
...
ggplot(data_membership_chances_both, 
  ...
  facet_wrap(~ Pantheon, 
             labeller = labeller(Pantheon = c("0" = "Centuries",
                                              "1" = "Pantheon")
                                 )
             )+ 
  xlab("year") + 
...
## Eras Defined as Separate Chapters in a History Course {#FHM-History}
...
kable(hpi_adjust_hist %>% as.data.frame() %>% head(50), format = "html") %>% 
  kable_styling(full_width = FALSE, position = "left") %>%
  scroll_box(width = "100%")
...
ggplot(data_membership_chances_both, 
  ...
  facet_wrap(~ Pantheon, 
             labeller = labeller(Pantheon = c("0" = "History class",
                                              "1" = "Pantheon")
             )
  )+ 
  xlab("year") + 
...
ranks_long = ranks %>%
  pivot_longer(cols = prop_hist:prop_peers, names_to = c("props")) %>%
  mutate(props = as.factor(props))
levels(ranks_long$props) = c("History class", "Centuries")
...
ggplot(ranks, aes(x = top)) +
  ...
  labs(title = "Proprtion of membership in Pantheon list (top 100)",
       subtitle = "Comparison of centuries and history class era definitions",
       x = "top x in Pantheon list",
       y = "proportion of agreement with Pantheon list") +
  ylim(0, 1) +
  geom_line(data = ranks_long, aes(x = top, y = value, color = props)) +
...
# Interesting Changes in Rankings {#Changes}

We have already discussed how each era-adjusted rankings of historical figures agree with the original Pantheon ranking list, and the differences in historical figures included in each top 50 list. Here we take a closer look at how ranking lists change when era-adjustment is applied. [Back to topics](#topics)

## Eras Defined as Centuries {#Changes-Centuries}
...
foo = combined_vs_peers[1:50, ] %>% 
  mutate(diff = rank_Pantheon - rank_vs_peers) %>% 
  arrange(desc(diff)) %>% 
  head(10)
kable(foo, format = "html") %>% 
  kable_styling(full_width = FALSE, position = "left") %>%
  scroll_box(width = "100%")

## penalized by era-adjustment
foo = combined_vs_peers %>% 
...
## Eras defined as Separate Chapters in a History Course {#Changes-History}
...
# Sensitivity in Eligible Population {#Sensitivity-Eligible}

Here we investigate how the results change when we change the historic eligible population. Our attention is when eras are defined by centuries. [Back to topics](#topics)


## Linear Increase in Eligible Population {#Linear-Increase}

Here we investigate how the historic rankings change when the historic eligible population exhibits a linear increase instead of an exponential increase as [originally calculated](#Centuries).


```{r}
n_gen = length(Epop_generations)
m1 = lm( Epop_generations[c(1,n_gen-1)] ~ c(1,n_gen-1) )
Epop_generations_linear =  c(coef(m1)[1] + coef(m1)[2] * 1:(n_gen-1), Epop_generations[n_gen])
```


This alternate historic eligible population is displayed below.


```{r}
cum_prop_population_linear = cumsum(Epop_generations_linear) / sum(Epop_generations_linear)
gen_stats_linear = data.frame(
  gen_start = generations,
  gen_end = c(generations[-1] -1, 1980),
  gen_pop = Epop_generations_linear, 
  cum_prop_population = cum_prop_population_linear, 
  n = sapply(hpi_generations, nrow)
)

ggplot(gen_stats_linear) + 
  aes(x = gen_start, y = gen_pop) +  
  geom_bar(stat = "identity") + 
  scale_x_continuous(breaks = seq(-800, 2000, by = 400)) + 
  theme_minimal(base_size = 14) + 
  labs(x = "year", y = "population",
       title ="Historical eligible population for each century (linear increase)", 
       subtitle = "800 BC to 1980 AD")
```


We now compute latent talent estimates using Full House Modeling with respect to this alternate historic eligible population.


```{r linear-pop-centuries, cache=TRUE}
system.time({
  hpi_adjust_vs_peers_linear = 
    do.call(rbind, 
            mclapply(seq_along(hpi_generations), 
                     mc.cores = nCores, function(j){
                       cbind(hpi_generations[[j]], 
                             "hpi_talent" = 
                               talent_computing_nonpara(ystar = as.numeric(ystar_mat[j, 1]), 
                                                        y = hpi_generations[[j]] %>% pull(hpi), 
                                                        npop = Epop_generations_linear[[j]]))
                     })) %>% arrange(desc(hpi_talent))
})
```


Here is the top 50 list of the most impactful people in history ranked by estimated latent talent obtained via Full House Modeling with respect to this alternate historic eligible population.


```{r}
kable(hpi_adjust_vs_peers_linear %>% as.data.frame() %>% head(50), 
      format = "html") %>% 
  kable_styling(full_width = FALSE, position = "left") %>%
  scroll_box(width = "100%")
```
...
This top 50 list has 40 names in common with the [original list with centuries used to designate eras](#FHM-Centuries), suggesting respectable sensitivity.


```{r}
common = 
  intersect(hpi_adjust_vs_peers_linear %>% as.data.frame() %>% head(50) %>% pull(name),
            hpi_adjust_vs_peers %>% as.data.frame() %>% head(50) %>% pull(name))
length(common)
```
[Back to topics](#topics)


## Flat Increase in Eligible Population {#Flat-Increase}

Here we investigate how the historic rankings change when the historic eligible population exhibits a linear increase, but is mostly flat, instead of an exponential increase as [originally calculated](#Centuries).


```{r}
Epop_generations_flat = Epop_generations[1] + 1000000 * 0:(n_gen-1)
```
...
```{r}
cum_prop_population_flat = cumsum(Epop_generations_flat) / sum(Epop_generations_flat)
gen_stats_flat = data.frame(
  gen_start = generations,
  gen_end = c(generations[-1] -1, 1980),
  gen_pop = Epop_generations_flat, 
  cum_prop_population = cum_prop_population_flat, 
  n = sapply(hpi_generations, nrow)
)

ggplot(gen_stats_flat) + 
  aes(x = gen_start, y = gen_pop) +  
  geom_bar(stat = "identity") + 
  scale_x_continuous(breaks = seq(-800, 2000, by = 400)) + 
  theme_minimal(base_size = 14) + 
  labs(x = "year", y = "population",
       title ="Historical eligible population for each century (flat increase)", 
       subtitle = "800 BC to 1980 AD")
```
...
```{r flat-linear-pop-centuries, cache=TRUE}
system.time({
  hpi_adjust_vs_peers_flat = 
    do.call(rbind, 
            mclapply(seq_along(hpi_generations), 
                     mc.cores = nCores, function(j){
                       cbind(hpi_generations[[j]], 
...
```{r}
kable(hpi_adjust_vs_peers_flat %>% as.data.frame() %>% head(50), 
      format = "html") %>% 
  kable_styling(full_width = FALSE, position = "left") %>%
  scroll_box(width = "100%")
```
...
This top 50 list has 31 names in common with the [original list with centuries used to designate eras](#FHM-Centuries), which is fewer than a linear increase as to be expected.


```{r}
common = 
  intersect(hpi_adjust_vs_peers_flat %>% as.data.frame() %>% head(50) %>% pull(name),
            hpi_adjust_vs_peers %>% as.data.frame() %>% head(50) %>% pull(name))
length(common)
```
[Back to topics](#topics)
